...
export default function ActivationForm() {
  ...
  // Autofill logic for existing users
  useEffect(() => {
    if (!isExisting) return;

    const timer = setTimeout(async () => {
      if (formData.username || formData.email || formData.telp) {
        try {
          ...
          if (res && res.found && res.data) {
            setFormData(prev => {
              ...
              fields.forEach(key => {
                if (res.data[key] && prev[key] !== res.data[key]) {
                  // Don't overwrite if user is actively typing in it, unless it's empty
                  if (!prev[key] || (key !== 'username' && key !== 'email' && key !== 'telp')) {
              ...
              if (updated) {
                // Show toast but don't cause render issues
                setTimeout(() => showToast("Data ditemukan, otomatis disalin.", "success"), 100);
              }
              return newData;
            });
          }
  ...
  }, [formData.username, formData.email, formData.telp, isExisting]);

  // Duplicate check logic for new users
  useEffect(() => {
    if (isExisting !== false) {
  ...
  const validateStep3 = () => {
    ...
    vehicles.forEach((vehicle, idx) => {
      ...
      if (vehicle.bodyType === "8" && !vehicle.customBodyType) {
        newErrors[`vehicle_${idx}_customBodyType`] = "Sebutkan nama body type.";
      }
    });
  ...
  // Excel Downloader using xlsx
  const downloadExcelTemplate = async () => {
    // Dynamically import xlsx so it doesn't bloat the initial bundle
    const XLSX = await import('xlsx');

    // Header row as requested: No. Rangka/Chassis | No. Polisi | Odometer | Metode Pembayaran | Body Type (optional)
    const header = [
      ["No. Rangka/Chassis", "No. Polisi", "Odometer", "Metode Pembayaran", "Body Type (optional)"]
    ];

    // Example data
    const data = [
      ["MHMFE75EKTK0XXXXX", "B1234XX", "5200", "CASH", "Box"],
      ["MHMFE71EHSK0XXXXX", "D5678YY", "12400", "PT DIPO STAR FINANCE", "Bak"]
    ];

    const ws = XLSX.utils.aoa_to_sheet([...header, ...data]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Template");
    XLSX.writeFile(wb, "Template_Aktivasi_GPS_Runner.xlsx");
  };

  // File Upload Handlers
  const fileInputRef = useRef(null);

  const processExcelPreview = async (file) => {
    try {
      const XLSX = await import('xlsx');
      ...
      reader.onload = (e) => {
        ...
        if (!isHeaderValid) {
          showToast("Format file tidak sesuai! Mohon pastikan header (judul kolom) sama persis dengan file template yang kami sediakan.");
          setExcelFile(null);
          setExcelPreviewData([]);
          setErrors(prev => ({ ...prev, excelFile: "Format kolom pada file Excel tidak sesuai dengan template." }));
        ...
        if (dataRows.length === 0) {
          showToast("File Excel kosong. Mohon isi data kendaraan sebelum diunggah.");
          setExcelFile(null);
          setExcelPreviewData([]);
          setErrors(prev => ({ ...prev, excelFile: "File Excel kosong." }));
          if (fileInputRef.current) fileInputRef.current.value = "";
          return;
        }
        ...
        const rangkaSet = new Set();
        const nopolSet = new Set();
        ...
        for (let i = 0; i < dataRows.length; i++) {
          ...
          const rangka = row[0] ? String(row[0]).trim() : "";
          const nopol = row[1] ? String(row[1]).trim() : "";
          const odometer = row[2] ? String(row[2]).trim() : "";
          const payment = row[3] ? String(row[3]).trim() : "";

          // Check required fields
          if (!rangka || !nopol || !odometer || !payment) {
            hasEmptyRequiredFields = true;
            break;
          }

          // Check duplicates
          if (rangkaSet.has(rangka)) {
            duplicateFound = `No. Rangka "${rangka}" duplikat pada baris ${i + 2}.`;
            break;
          }
          if (nopolSet.has(nopol)) {
            duplicateFound = `No. Polisi "${nopol}" duplikat pada baris ${i + 2}.`;
            break;
          }

          rangkaSet.add(rangka);
          nopolSet.add(nopol);
        }

        if (hasEmptyRequiredFields) {
          showToast("Gagal unggah: Terdapat data wajib (No. Rangka, No. Polisi, Odometer, Metode Pembayaran) yang kosong.");
          setExcelFile(null);
          setExcelPreviewData([]);
          setExcelParsedData([]);
          setErrors(prev => ({ ...prev, excelFile: "Terdapat data wajib yang kosong di Excel." }));
          if (fileInputRef.current) fileInputRef.current.value = "";
          return;
        }

        if (duplicateFound) {
          showToast(`Gagal unggah: ${duplicateFound} Data tidak boleh ganda.`);
          setExcelFile(null);
          setExcelPreviewData([]);
          setExcelParsedData([]);
          setErrors(prev => ({ ...prev, excelFile: duplicateFound }));
          if (fileInputRef.current) fileInputRef.current.value = "";
          return;
        }

        // Parse to save full data for submission
        ...
        for (let i = 0; i < dataRows.length; i++) {
          ...
          parsedVehicles.push({
            rangka: row[0] ? String(row[0]).trim() : "",
            nopol: row[1] ? String(row[1]).trim() : "",
            odometer: row[2] ? String(row[2]).trim().replace(/\D/g, "") : "0",
            payment: row[3] ? String(row[3]).trim() : "",
            bodyType: row[4] ? String(row[4]).trim() : ""
          });
        }
        setExcelParsedData(parsedVehicles);

        // Set state for valid file
        setExcelFile(file);
        setErrors(prev => ({ ...prev, excelFile: null }));
        // Take the first 6 rows for preview (1 header + 5 data rows)
        setExcelPreviewData(jsonData.slice(0, 6));
      };
      ...
      setExcelPreviewData([]);
      setExcelParsedData([]);
      setErrors(prev => ({ ...prev, excelFile: "Terjadi kesalahan saat membaca file. Pastikan file tidak rusak." }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      // Map ID/Code to String Name for DB Storage
      const mappedFormData = {
        ...formData,
        sektor: formData.sektor ? getSektorName(formData.sektor) : "",
        provinsi: formData.provinsi ? getProvinceName(formData.provinsi) : "",
        kabupaten: formData.kabupaten ? getRegencyName(formData.provinsi, formData.kabupaten) : "",
        kecamatan: formData.kecamatan ? getDistrictName(formData.kabupaten, formData.kecamatan) : "",
        kelurahan: formData.kelurahan ? getVillageName(formData.kecamatan, formData.kelurahan) : ""
      };

      let finalVehicles = [];
      if (formData.jumlah === "5") {
        if (!excelFile || excelParsedData.length === 0) {
          setIsLoading(false);
          setErrors(prev => ({ ...prev, excelFile: "Harap unggah file Excel yang valid." }));
          return;
        }
        finalVehicles = excelParsedData;
      } else {
        finalVehicles = vehicles.map(v => ({
          ...v,
          payment: v.payment ? getPaymentName(v.payment, v.customPayment) : "",
          bodyType: v.bodyType ? getBodyTypeName(v.bodyType, v.customBodyType) : ""
        }));
      }
  ...
  const getBodyTypeName = (id, customText) => {
    if (!id) return "-";
    if (id === "8") return `LAINNYA: ${customText || ""}`;
    return BODY_TYPE_OPTIONS.find(p => p.id === id)?.name || "-";
  };
    ...
    <>
      ...
      <header className={`bg-white border-b border-slate-200 px-6 fixed top-0 left-0 right-0 z-50 shadow-sm transition-all duration-300 ${isScrolled ? 'py-2' : 'py-4'}`}>
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          >
            <span className={`w-2 h-2 rounded-full ${serverStatus === 'online' ? 'bg-emerald-500 animate-pulse' : serverStatus === 'connecting' ? 'bg-amber-500 animate-pulse' : 'bg-rose-500'} group-hover:hidden`}></span>
            <span className="group-hover:hidden">{serverStatus === 'online' ? 'Server Live Terhubung' : serverStatus === 'connecting' ? 'Menghubungkan...' : 'Server Offline'}</span>
            <span className="hidden group-hover:inline font-bold">Menuju Dashboard</span>
          </div>
        </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 p-6 md:p-10 transition-all duration-300">
              ...
              {step === 2 && (
                <div className="space-y-6 animate-fadeIn">
                  ...
                  {duplicateWarning && (
                    <div className="bg-rose-50 border border-rose-200 p-4 rounded-xl flex items-start space-x-3 text-rose-800 animate-fadeIn mb-6">
                      <AlertTriangle className="w-5 h-5 text-rose-600 shrink-0 mt-0.5" />
                      <div>
                        <h4 className="font-bold text-sm">Data Sudah Terdaftar!</h4>
                        <p className="text-xs mt-1 leading-relaxed opacity-90">
                          Username, Email, atau No. Telepon yang Anda masukkan sudah terdaftar di sistem kami. Jika Anda adalah pemilik akun ini, apakah Anda ingin beralih ke formulir penambahan unit?
                        </p>
                        <button
                          onClick={() => {
                            setIsExisting(true);
                          ...
                          className="mt-3 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-lg transition-colors"
                        >
                          Beralih ke Penambahan Unit
                        </button>
                      </div>
                    </div>
                  )}
                      ...
                      {vehicles.map((v, idx) => (
                        <div key={v.id} className="border border-slate-100 rounded-2xl bg-slate-50/50 p-5 md:p-6 space-y-4 shadow-sm relative">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {/* Body Type */}
                            <div className="space-y-1">
                              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Body Type (Opsional)</label>
                              {v.bodyType === "8" ? (
                                <div className="relative animate-fadeIn">
                                  <input
                                    type="text"
                                    value={v.customBodyType || ""}
                                    onChange={(e) => handleVehicleChange(idx, "customBodyType", e.target.value, formatUppercase)}
                                    placeholder="Ketik Body Type..."
                                    autoFocus
                                    className={`w-full pl-4 pr-10 py-3 bg-slate-50 border rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-bold placeholder:font-normal text-sm ${errors[`vehicle_${idx}_customBodyType`] ? 'border-rose-500 bg-rose-50/10' : 'border-slate-200'
                                      }`}
                                  />
                                  <button
                                    type="button"
                                    onClick={() => {
                                      handleVehicleChange(idx, "bodyType", "");
                                      handleVehicleChange(idx, "customBodyType", "");
                                    }}
                                    className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 bg-slate-200/80 hover:bg-slate-300 rounded-full p-1 transition-colors"
                                ...
                                <CustomSelect
                                  value={v.bodyType || ""}
                                  onChange={(e) => handleVehicleChange(idx, "bodyType", e.target.value)}
                                  options={BODY_TYPE_OPTIONS.map(opt => ({ value: opt.id, label: opt.name }))}
                                  placeholder="Pilih Body Type"
                                  error={errors[`vehicle_${idx}_bodyType`]}
                                />
                              )}

                              {v.bodyType === "8" ? (
                                errors[`vehicle_${idx}_customBodyType`] && (
                                  <p className="text-rose-500 text-[11px] font-bold mt-1">{errors[`vehicle_${idx}_customBodyType`]}</p>
                                )
                              ) : (
                                errors[`vehicle_${idx}_bodyType`] && (
                                  <p className="text-rose-500 text-[11px] font-bold mt-1">{errors[`vehicle_${idx}_bodyType`]}</p>
                                )
                              )}
                            </div>
                      ...
                      {excelFile && (
                        <div className="space-y-4">
                          ...
                          {excelPreviewData.length > 0 && (
                            <div className="border border-slate-200 rounded-xl overflow-hidden bg-white animate-fadeIn">
                              <div className="bg-slate-50 px-4 py-3 border-b border-slate-200">
                                <span className="text-xs font-bold text-slate-600">Pratinjau Data (Maks {excelPreviewData.length - 1} Baris)</span>
                              </div>
                              <div className="overflow-x-auto">
                                <table className="w-full text-left text-[11px] whitespace-nowrap">
                                  <thead>
                                    <tr className="bg-slate-50 text-slate-500">
                                      {excelPreviewData[0].map((h, i) => (
                                        <th key={i} className="px-4 py-2 font-semibold border-b border-slate-200">{h}</th>
                                      ))}
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {excelPreviewData.slice(1).map((row, rIndex) => (
                                      <tr key={rIndex} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                                        {excelPreviewData[0].map((_, cIndex) => (
                                          <td key={cIndex} className="px-4 py-2.5 text-slate-700 font-medium">{row[cIndex] || "-"}</td>
                                        ))}
                                      </tr>
                                    ))}
                                  </tbody>
                            ...
                            <table className="w-full text-left text-sm border-collapse">
                              <thead>
                                <tr className="border-b border-slate-200 text-slate-400 font-bold">
                                  ...
                                  <th className="pb-3 font-semibold text-xs uppercase">Body Type</th>
                              ...
                              <tbody>
                                {vehicles.map((v, index) => (
                                  <tr key={v.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                                    ...
                                    <td className="py-3.5 font-bold text-slate-700">{getBodyTypeName(v.bodyType, v.customBodyType)}</td>
              ...
              <div className="flex items-center justify-between border-t border-slate-100 pt-6 mt-8">
                ...
                {step < 4 ? (
                  <button
                    ...
                    className={`flex items-center space-x-2 font-bold py-3 px-8 rounded-xl transition duration-200 shadow-lg ${serverStatus === 'offline' ? 'bg-slate-300 text-slate-500 cursor-not-allowed shadow-none' : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-100'}`}
                  >
                    <span>Lanjutkan</span>
                  ...
                  <button
                    ...
                    disabled={!agreeTerms || serverStatus === 'offline'}
                    className={`flex items-center space-x-2 font-bold py-3 px-10 rounded-xl transition duration-200 shadow-lg ${serverStatus === 'offline' ? 'bg-slate-300 text-slate-500 cursor-not-allowed shadow-none' : agreeTerms
              ...
              {serverStatus === 'offline' && (
                <div className="mt-4 p-4 bg-rose-50 border border-rose-200 rounded-xl flex items-start space-x-3 text-rose-700 animate-fadeIn">
                  <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />
                  <p className="text-sm font-medium">
                    Koneksi ke server saat ini sedang terputus. Pendaftaran sementara tidak dapat dilanjutkan. Silakan hubungi dealer untuk bantuan lebih lanjut.
                  </p>
                </div>
              )}
      ...
      {/* Custom Toast Alert */}
      <div className={`fixed top-4 right-4 md:top-6 md:right-6 z-[9999] transition-all duration-300 transform ${toast.show ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0 pointer-events-none'}`}>
        <div className={`flex items-start space-x-3 p-4 rounded-xl shadow-lg shadow-rose-500/10 border max-w-sm ${toast.type === 'error' ? 'bg-rose-50 border-rose-200 text-rose-800' : 'bg-emerald-50 border-emerald-200 text-emerald-800'}`}>
          {toast.type === 'error' ? <AlertTriangle className="w-5 h-5 text-rose-500 shrink-0 mt-0.5" /> : <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />}
          <div className="flex-1">
            <p className="text-sm font-semibold leading-relaxed">{toast.message}</p>
          </div>
          <button onClick={() => setToast(prev => ({ ...prev, show: false }))} className="text-slate-400 hover:text-slate-600 shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
      </div>
